        ...
        $(this).resizable({
            ...
            stop: function() {
                ...
                setTimeout(function() {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "../rapidpubapp/ajax/magnets_size.php",
                        dataType: "json",
                        data: {
                            action: 'getsize',
                            ...
                            vitWidthpx: $('#magnet_content').width(),
                            vitHeightpx: $('#magnet_content').height(),
                            vitWidthcm: $('#longueur').val(),
                            vitHeightcm: $('#hauteur').val(),
                            vitType: $.data(document.body, 'typemagnet'),
                            vitroType: $.data(document.body, 'typemagnetVitro'),
                            type: elemType
                        },
...
$(function() {
    ...
    $.data(document.body, 'currentImage', false); $.data(document.body, 'typemagnet', false); $.data(document.body, 'showColors', false);
    $.data(document.body, 'selfHeight', false); $.data(document.body, 'textEdit', false); $.data(document.body, 'typemagnetVitro', false);
    ...
    var tour = {
        ...
        steps: [{
            title: "Dimensions",
            content: "Saisissez les dimensions (longueur/hauteur) de votre magnet en centimètres. Cette étape est importante car elle nous permet de vous proposer un tarif sur mesure",
            target: document.querySelector("#block_right"),
            placement: "left"
        }, {
            title: "Texte et police d'écriture",
            content: "Ajoutez des textes sur votre magnet. Cliquez sur Ok ou appuyez sur la touche Entrée de votre clavier pour valider votre saisie.",
            ...
            content: "Ce bouton sert à dessiner des formes carrés ou rectangulaires sur votre magnet. Vous pouvez utiliser la combinaison 'MAJ + redimensionnement souris' pour conserver les proportions.",
            ...
            content: "Ces boutons de contrôles vous permettent d'interagir avec l'élement sélectioné (entouré en rouge sur l'aperçu), vous pouvez le déplacer, le centrer sur votre magnet, le dupliquer ou même le faire pivoter.",
    ...
    $('#drawRect').click(function(e) {
        ...
        if (setCfg == false) {
            swal({
                title: "Attention",
                text: "Vous devez saisir les dimensions de votre magnet avant de pouvoir continuer !",
                type: "warning",
                closeOnConfirm: true
            }, function() {
        ...
        $.ajax({
            type: "POST",
            url: "../rapidpubapp/ajax/magnets_forme.php",
            dataType: "json",
            data: {
                action: 'update',
                couleur: '#000000',
            ...
            success: function(result) {
                ...
                setTimeout(function() {
                    var maxWidth = $('#magnet_content').width() - 50;
                    var maxHeight = $('#magnet_content').height() - 50;
                    var ratio = 0;
                ...
                setTimeout(function() {
                    $.ajax({
                        type: "POST",
                        url: "http://rapidpub.synapdev.fr/rapidpubapp/ajax/magnets_size.php",
                        dataType: "json",
                        data: {
                            action: 'getsize',
                            id: result['id'],
    ...
    $('#magnet_content, #magnet_bg').click(function(e) {
        e.preventDefault();
                    ...
                    if (resultx) {
                        ...
                        if ( typeElem == 'fond' ) {
                            ...
                            if ( typeElem == 'forme' ) {
                                ...
                                $('.selectedElement').find('.magnet_element').attr('id', newid);
                            }                            
                            zIndex = zIndex + 1;
                            $('.selectedElement').css('z-index', zIndex);
                            var nbrLayers = $('ul#calques li').length;
            ...
            success: function(result) {
                ...
                if (result.success == true) {
                    ...
                    swal({
                        ...
                        if (isConfirm) {
                            $('#magnet_content').append('<div class="elementContainer typeImage resizable draggable rotatable" style="max-height:100%;position:absolute"><div class="reset_rotate ctrlElem" title="Réinitialiser la position de l\'élement"></div><img class="elemContent magnet_element" style="max-height:100%" src="' + result.imgPathOp + '" alt="' + result.imgPathOp + '" id="' + result.id + '" /><div class="ctrlElem seresizer ui-resizable-handle ui-resizable-se"></div><div class="ctrlElem delete-element" title="Supprimer l\'élément"></div></div>');
                            setTimeout(function() {
                                var maxWidth = $('#magnet_content').width() - 50;
                                var maxHeight = $('#magnet_content').height() - 50;
                                var ratio = 0;
    ...
    $('.dimensions_magnet').keyup(function() {
        var typeSel = $(this).val();
        if (isNumber(typeSel)) {
            if ($('#hauteur').val() > 60 && $('#longueur').val() > 60) {
                swal({
                    title: "Attention",
                    html: true,
                    text: "<p style='text-align:justify'>Nous ne pouvons imprimer que des magnets rectangulaires où le plus petit côté <strong>ne doit pas excéder 60 cm</strong>.<br>Exemple: 100 x 60 pour un magnet horizontal ou 60 x 100 pour un magnet vertical.</p>",
                    type: "error",
                    closeOnConfirm: true
                }, function() {
                    $('#longueur').effect('highlight', {
            ...
            if (longueur > hauteur) {
                ...
                if (newHeight <= 500) $('#magnet_content').width(750).height(newHeight);
                else $('#magnet_content').width(500 / ratio).height(500);
            } else if (hauteur > longueur) {
                var ratio = longueur / hauteur;
                $.data(document.body, 'ratioCfg', ratio);
                var newWidth = parseInt(500) * ratio;
                if (newWidth <= 750) $('#magnet_content').height(500).width(500 * ratio);
                else $('#magnet_content').height(750 / ratio).width(750);
            } else if (hauteur == longueur) {
                $('#magnet_content').height(500).width(500);
            }
            $('#magnet_content').center(true);
            if (isNumber($('#longueur').val()) && isNumber($('#hauteur').val())) {
                setCfg = true;
                var vitWidth = $('#magnet_content').width(),
                    vitHeight = $('#magnet_content').height();
                $('#magnet_height').html('<span style="color:#ff0000;font-size:10px;">' + hauteur + '</span>').css('height', vitHeight);
                $('#magnet_width').html('<span style="color:#ff0000;font-size:10px;">' + longueur + '</span>').css('width', vitWidth);
                var magnetsy = $('#magnet_content').offset();
                $('#magnet_width').css({
                    top: -15,
                    left: 0
                });
            ...
            $.ajax({
                ...
                success: function(result) {
                    ...
                    $.each(result, function(i, item) {
                        $('#motifs_container').append('<div title="Ajouter un motif sur votre magnet" class="motif" id="' + item['code'] + '" style="background-size:100%;background-image: url(' + item['path'] + ');"></div>');
                    });
                    var width = 0;
        ...
        onSelected: function(index, value, text) {
            ...
            if ($('.selectedElement').hasClass('typeText')) {
                ...
                $.ajax({
                    type: "POST",
                    dataType: "json",
                    url: "../rapidpubapp/ajax/magnets_text.php",
                    data: {
                        action: 'update',
    ...
    $('body').on('click', '.draggable', function(e) {
        ...
        if ($(this).hasClass('typeText')) {
            $.data(document.body, 'currentID', $(this).find('.magnet_element').attr('id'));
    ...
    $('#longueur, #hauteur').on('input', function() {
        var elemCount = $('#magnet_content > .elementContainer').length;
        if (elemCount >= 1 && setCfg) {
            swal({
                title: "Attention",
                text: "Vous perdrez votre travail si vous changez les dimensions de la magnet et vous devrez recommencer votre création",
                type: "warning",
                ...
                if (isConfirm) {
                    $('#calques > li').remove();
                    $('#magnet_content .elementContainer').remove();
    ...
    $('#add_text').click(function(e) {
        ...
        var parentSize = $('#magnet_content').css('width');
        var supportWidth = $('#longueur').val();
        var supportHeight = $('#hauteur').val();
        if (textVal == '') {
            swal({
            ...
            $.ajax({
                ...
                success: function(result) {
                    ...
                    $('.selectedElement').removeClass('selectedElement');
                    $('#magnet_content').append('<div class="elementContainer typeText resizable draggable rotatable" data-colorcode="#000000" style="position:absolute"><div class="reset_rotate ctrlElem" title="Réinitialiser la position de l\'élement"></div><img class="magnet_element elemContent" src="data:image/png;base64,' + result['image'] + '" data-ccode="' + couleurVar + '" alt="' + result['text'] + '" id="' + result['id'] + '" /><div class="ctrlElem seresizer ui-resizable-handle ui-resizable-se"></div><div class="delete-element ctrlElem" title="Supprimer l\'élément"></div></div>');
                    $('#elemColor').colpickSetColor('#000000');
                    $('#colorpicker-element').stop( true ).animate({height:205},200);
                    $('#' + result['id']).parent('div').addClass('selectedElement');
                    $('input.elemSize').prop('disabled', false);
    ...
    $('body').on('click', '.motif', function() {
        ...
        $.ajax({
            type: "POST",
            url: "../rapidpubapp/ajax/magnets_motif.php",
            dataType: "json",
            data: {
                action: 'update',
                motif: motifUrl,
    ...
    $('.colorblock').click(function(){
        ...
        if ($('.selectedElement').hasClass('typeMotif')) {
            ...
            $.ajax({
                type: "POST",
                dataType: "json",
                url: "../rapidpubapp/ajax/magnets_fond.php",
                data: {
                    action: 'update',
                    couleur: color,
                    type: 'fond',
    ...
    $('.colorblockfond').click(function(){
        var color = $(this).attr('data-hexcolor');
        $('#magnet_content').css('background-color', color);
    });
    $('#bgColor').colpick({
        flat: true,
        color: '000000',
    ...
    $('#epaisseur').change(function() {
        ...
        var parentWidth = $('#magnet_content').width();
        if ($('#magnet_content #bordure').length == 0) {
            $('#magnet_content').append('<div id="bordure" style="width:100%;height:100%;"><div style="width:100%;height:100%;" id="bordure_interne" style="box-shadow: inset 0 0 0 ' + epaisseur + 'px #000000;"></div></div>');
        } else {
